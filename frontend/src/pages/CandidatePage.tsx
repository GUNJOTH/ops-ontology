...
export function CandidatePage() {
  ...
  const query: CandidateQuery = useMemo(() => ({
    ...
    classification: params.get('classification') ?? undefined, quickFilter: (params.get('quick_filter') as CandidateQuery['quickFilter']) ?? 'all',
  }), [params])
  ...
  return <div>
    <PageHeader title="设备候选" description="只处理当前高质量批次，搜索与筛选结果保存在 URL" extra={<><Tag color="blue">{candidates.data?.total.toLocaleString() ?? '…'} 条</Tag><ReadOnlyTag /></>} />
