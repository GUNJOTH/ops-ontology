...
CORE_OBJECT_TYPES = (
    ("organization", "组织", "组织业务对象根类型", None, "abstract", 0),
    ("company", "分公司", "组织下的分公司", "organization", "entity", 1),
    ("center", "中心", "负责站点和设备的中心组织", "organization", "entity", 1),
    ("team", "责任班组", "负责巡检、消缺或工单执行的班组", "center", "entity", 1),
    ("physical_object", "物理对象", "站点和设备的物理对象根类型", None, "abstract", 0),
    ("site", "站点", "设备运行所属站点", "physical_object", "entity", 1),
    ("device", "设备", "由源域身份唯一定位的设备业务对象", "physical_object", "entity", 1),
    ("location", "位置", "设备所在的功能位置和位置层级", "physical_object", "entity", 1),
    ("specialty", "专业", "设备、缺陷和工单对应的专业分类", "organization", "entity", 1),
    ("business_event", "业务事件", "巡检、缺陷、消缺和工单事件根类型", None, "abstract", 0),
    ("inspection", "巡检", "一次巡检业务记录或过程对象", "business_event", "entity", 1),
    ("abnormal_inspection", "异常巡检", "巡检中产生且必须有测量或现象证据的异常对象", "inspection", "entity", 1),
    ("defect", "缺陷", "经业务流程登记或确认的缺陷对象", "business_event", "entity", 1),
    ("repeated_defect", "重复缺陷", "由时间窗、对象和相似证据派生的判断对象", "defect", "knowledge", 1),
    ("severe_defect", "严重缺陷", "由风险因素和规则派生的严重缺陷判断对象", "defect", "knowledge", 1),
    ("defect_resolution", "消缺", "对缺陷采取的处理和验收对象", "business_event", "entity", 1),
    ("work_order", "工单", "组织执行维护工作的工单对象", "business_event", "entity", 1),
    ("work_permit", "工作票", "作业许可、风险和安全措施对象", "business_event", "entity", 1),
    ("human_review", "人工审核", "对事实、判断或行动计划的治理审核对象", "business_event", "entity", 1),
    ("business_knowledge", "业务知识", "标准、规则、SOP 和风险知识根类型", None, "abstract", 0),
    ("knowledge_asset", "知识资产", "具有身份、来源、版本和审核状态的知识资产", "business_knowledge", "knowledge", 1),
    ("standard", "标准", "业务标准知识资产", "business_knowledge", "knowledge", 1),
    ("rule", "规则", "可判断、可计算、可约束、可行动的规则", "business_knowledge", "knowledge", 1),
    ("sop", "SOP", "标准作业程序知识资产", "business_knowledge", "knowledge", 1),
...
def build(target_path: pathlib.Path) -> dict[str, object]:
    ...
    ensure_schema(db)
    created = now()
    for object_type, display_name, description, parent, kind, instantiable in CORE_OBJECT_TYPES:
        db.execute(
    ...
    for event_type, display_name, subject_type, affects_state, source_mapping in EVENT_TYPES:
        db.execute(
            """INSERT INTO ontology_event_type(event_type,display_name,subject_type,affects_state,source_mapping,version,review_status,status,created_at,updated_at)
              VALUES (?,?,?,?,?,'ontology-v1','approved','active',?,?)
              ON CONFLICT(event_type) DO UPDATE SET display_name=excluded.display_name,subject_type=excluded.subject_type,
                affects_state=excluded.affects_state,source_mapping=excluded.source_mapping,version=excluded.version,
                review_status='approved',status='active',updated_at=excluded.updated_at""",
            (event_type, display_name, subject_type, affects_state, source_mapping, created, created),
        )
    ...
    for row in event_rows:
        ...
        db.execute(
            """INSERT INTO ontology_event_type(event_type,display_name,subject_type,affects_state,source_mapping,version,review_status,status,created_at,updated_at)
              VALUES (?,?,?,0,'semantic_event','inferred-from-existing-v1','needs_review','active',?,?)
    ...
    for from_state, event_type, guard, to_state, priority in TRANSITIONS:
        ...
        db.execute(
            """INSERT INTO ontology_transition_rule(transition_rule_id,machine_id,from_state,event_type,guard_json,to_state,priority,version,
              ...
              ON CONFLICT(transition_rule_id) DO UPDATE SET from_state=excluded.from_state,event_type=excluded.event_type,
                guard_json=excluded.guard_json,to_state=excluded.to_state,priority=excluded.priority,updated_at=excluded.updated_at""",
            (transition_id, state_machine_id, from_state, event_type, json.dumps(guard, ensure_ascii=False, sort_keys=True), to_state, priority, created, created),
        )
    ...
    db.commit()
    db.close()
    return {"run_id": run_id, **counts, "unregistered_relation_count": unregistered_relations, "unregistered_event_count": unregistered_events, "source_write": False, "formal_publication": False}
